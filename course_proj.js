...
function handleButtonClick(event) {
    ...
    if (event.target.id === 'tab1') {
        // Reset styles for "Find Dates" tab to its original state
        contentTab1.style.display = 'block';
        // Assuming you want these elements to be visible when tab1 is active
        const dayOptionsContainer = document.getElementById('day-options-container');
        const resultsContainer = document.getElementById('results-container');
        const dayPresetButton = document.getElementById('day-preset-button');
        const monthPresetButton = document.getElementById('month-preset-button');
        const weekPresetButton = document.getElementById('week-preset-button'); // Assuming you have this
    
        // Reset display styles for these elements
        dayOptionsContainer.style.display = 'flex'; // Adjust based on your layout needs
        resultsContainer.style.display = 'block';
        dayPresetButton.style.display = 'block';
        monthPresetButton.style.display = 'block';
        weekPresetButton.style.display = 'block'; // Adjust this as well if you have a week preset button
    
        // Reset styles for the date-tab-content as needed
        document.querySelector('.date-tab-content').style.display = 'flex';
        document.querySelector('.date-tab-content').style.flexDirection = 'row'; // Adjust as needed
        document.querySelector('.date-tab-content').style.justifyContent = 'center'; // Adjust as needed
    } else if (event.target.id === 'tab2') {
        // Show "Holiday Info" tab content
        contentTab2.style.display = 'block';
        // Reset any specific styles applied to this tab's content when it was hidden
        // If there's any specific element that should be hidden when this tab is active, handle it here
    }

    // Keep the calendar (date inputs) always visible
    document.addEventListener('DOMContentLoaded', function() {
        const dateInputContainer = document.getElementById('date-input-container');
        if (dateInputContainer) { // Check if the element exists
...
const dayPresetButton = document.getElementById('day-preset-button');
const weekPresetButton = document.getElementById('week-preset-button');
const monthPresetButton = document.getElementById('month-preset-button');

dayPresetButton.addEventListener('click', () => setPresetDuration(6));
weekPresetButton.addEventListener('click', () => setPresetDuration(13));
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Set the min attribute for both start and end date inputs to today's date
    if (startDateInput) {
